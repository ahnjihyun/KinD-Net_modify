...
input_decom = tf.compat.v1.placeholder(tf.float32, [None, 400, 600, 3], name='input_decom')
#restoration input
input_low_r = tf.compat.v1.placeholder(tf.float32, [None, 384, 384, 3], name='input_low_r')
input_low_i = tf.compat.v1.placeholder(tf.float32, [None, 384, 384, 1], name='input_low_i')
input_high_r = tf.compat.v1.placeholder(tf.float32, [None, 384, 384, 3], name='input_high_r')
...
for idx in range(len(train_low_data)):
    ...
    RR0 = np.squeeze(RR)
    II0 = np.squeeze(II)
    print("idx, RR0, II0 : ", idx, RR0.shape, II0.shape)
...
for idx in range(len(eval_low_data)):
    ...
    RR03 = np.squeeze(RR3)
    II03 = np.squeeze(II3)
    print("idx, RR03, II03 : ", idx, RR03.shape, II03.shape)
...
print("len(train_restoration_high_r_data / low_r / low_i)", len(train_restoration_high_r_data), len(train_restoration_low_r_data),len(train_restoration_low_i_data))
print("len(eval_restoration_low_r_data / low_i)", len(eval_restoration_low_r_data),len(eval_restoration_low_i_data))
